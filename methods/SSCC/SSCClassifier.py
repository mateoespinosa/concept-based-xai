...
Contains implementations of the following:
    1) An abstract SSCClassifier class, specifying the requirements of a generic
...
class SSCClassifier(ABC):
    ...
    def fit(self, data_gen_l, data_gen_u):
        ...
        :param data_gen_u: Tensorflow Dataset, returning tuples of points
            (x_data, y_data) In the fully-unsupervised case (where y-labels are
            not available as well), y_data should be set to -1
...
class SSCC_CME(SSCClassifier):
    ...
    def __init__(self, **kwargs):
        ...
        params = {
            key: val
            for (key, val) in kwargs.items() if key != "base_model"
        }
    ...
    def fit(self, data_gen_l, data_gen_u):
        ...
        # training
        # Thus, we filter out the y-labels from data_gen_l and data_gen_u via a
        # mapping
    ...
    def predict(self, data_gen_u):
        ...
        return self.concept_predictor.predict_concepts(
            remove_ds_el(data_gen_u)
        )
...
class SSCC_CBM(SSCClassifier):
    ...
    def predict(self, data_gen_u):
        return self.concept_predictor.predict(remove_ds_el(data_gen_u))
...
class SSCC_Group_VAEArgmax(SSCClassifier):
    ...
    def _build_paired_dataset(self, ds):
        ...
        c_ids = np.array([
            latent_to_index(elem[1].numpy(), latents_bases)
            for elem in ds
        ])
        ...
        print("% pairs:", n_pairs/(n_pairs + n_non_pairs) * 100)

        return tf.data.Dataset.from_tensor_slices((x_pairs, label_ids))
        ...
        for i in range(0, n_epochs, frequency):
            ...
            for j in range(self.n_concepts):
                ...
                if self.n_train_predictor is not None:
                    z_train = z_train[:self.n_train_predictor]
                    c_train = c_train[:self.n_train_predictor]
                ...
                print(f"Accuracy of concept {j}: {accuracy}")
