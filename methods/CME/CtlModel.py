...
Class for the transparent model representing a function from concepts to task
labels.
Represents the decision-making process of a given black-box model, in the
...
class CtLModel:
    ...
    def __init__(self, **params):
        ...
        self.c_names = self.params.get(
            "concept_names",
            [f"Concept_{i}" for i in range(self.n_concepts)],
        )
        self.cls_names = self.params.get(
            "class_names",
            [f"Class_ {i}" for i in range(self.n_classes)],
        )
    ...
    def train(self, c_data, y_data):
        ...
        if self.clf_type == 'DT':
            ...
            raise ValueError(f"Unrecognised model type {self.clf_type}...")
