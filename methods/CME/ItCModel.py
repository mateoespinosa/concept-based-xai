...
class ItCModel(object):
    ...
    def __init__(self, model, **params):
        ...
        self.layer_ids = self.layer_ids or list(range(len(model.layers)))

        # Retrieve the corresponding layer names
        self.layer_names = params.get(
            "layer_names",
            [f"Layer_{i}" for i in range(len(model.layers))],
        )
        ...
        self.concept_names = params.get(
            "concept_names",
            [f"Concept_{i}" for i in range(self.n_concepts)],
        )
    ...
    def get_clf(self):
        if self.clf_method == 'LR':
            ...
            raise ValueError(f"Unimplemented method {self.clf_method}")
    ...
    def train(self, ds_l, ds_u):
        ...
        for i, layer_id in enumerate(self.layer_ids):
            print("-" * 20)
            print(
                f"Processing layer {i + 1}/{len(self.layer_ids)}:",
            ...
            print(f"Processed layer {i + 1}/{len(self.layer_ids)}")

        # Initialise concept predictors with models in the first layer
        # Consists of an array of size |concepts|, in which
        # The element arr[i] is the Layer Id of the layer to use when
        # predicting that concept
