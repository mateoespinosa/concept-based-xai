...
class ConceptBottleneckModel(object):
    ...
    def __init__(
        ...
        :param layer_id: layer_id of the model layer to use as the output
            activations to feed into the bottleneck
        :param n_c_vals_list: list of length (n_concepts), where
            n_c_vals_list[i] denotes the number of concept values of concept_i
        :param multi_task: whether to setup a separate softmax output for each
            concept (i.e. a multi-task setup), or whether to simply include all
            concept values as a sigmoid layer (i.e. a multi-label setup)
        :param end_to_end: whether to create an end-to-end Keras model (with
            the concept model being part of the model), or keep as separate
            models
        :param c_epochs: number of epochs to use during training of the concept
            extractor
        ...
        self.offsets = tf.convert_to_tensor(np.array([
            sum(self.n_c_vals_list[:i]) for i in range(len(n_c_vals_list))
        ]))
        self.n_total_c_vals = sum(self.n_c_vals_list)

        self.c_optimizer = c_optimizer or tf.keras.optimizers.Adam(
            learning_rate=0.001
        )
    ...
    def _build_concept_extractor(self, model):
        ...
        base = tf.keras.Model(
            inputs=model.inputs,
            outputs=model.layers[self.layer_id].output,
        )
        x = base.output
        ...
        if self.end_to_end:
            ...
            if self.multi_task:
                raise NotImplementedError(
                    "Currently, multi-task label predictor not implemented..."
                )

            else:
                # TODO: currently, create a 2-layer MLP
                # TODO: in the future, consider passing in an arbitrary model as
                #       your LP
                in_tensor = Input(self.concept_extractor.output.shape[1:])
                output = Dense(2*self.n_classes, activation='relu')(in_tensor)
                output = Dense(
                    self.n_classes,
                    activation='softmax',
                    name='lp',
                )(output)
                label_predictor = tf.keras.Model(
                    inputs=in_tensor,
                    outputs=[output],
                )
    ...
    def _build_cb_model(self):
        ...
        if self.end_to_end:
            ...
            all_losses = {
                cp_names[i]: self.concept_extractor_losses[i]
                for i in range(len(cp_names))
            }
            all_losses = {
                **all_losses,
                **{lp_name: self.label_predictor_loss}
            }
            all_metrics = {
                cp_names[i]: self.concept_extractor_metrics[i]
                for i in range(len(cp_names))
            }
            all_metrics = {
                **all_metrics,
                **{lp_name: self.label_predictor_metric},
            }
            loss_weights = {
                cp_name: 1.0
    ...
    def train(self, data_gen_l):
        '''
        :param data_gen_l: tf.dataset generator, returning tuples (x, c, y) for
            image, concept, and label data
        '''
        ...
        if self.end_to_end:
            if self.multi_task:
                raise NotImplementedError(
                    "Multi-task end-to-end learning not implemented yet..."
                )

            else:
                ds_l = data_gen_l.map(
                    lambda x, c, y: (x, tuple([self.binarize_map(c), y]))
                )
                ...
                self.cb_model.fit(ds_l_train.batch(
                    self.batch_size),
            ...
            if self.multi_task:
                ...
                ds_l = data_gen_l.map(
                    lambda x, c, y: (x, self.convert_to_multioutput(c))
                )
            else:
                # Binarize concept values, in case of the multi-label setup
                ds_l = data_gen_l.map(
                    lambda x, c, y: (x, self.binarize_map(c))
                )
            ...
            self.concept_extractor.fit(
                ds_l_train.batch(self.batch_size),
                epochs=self.n_epochs,
                validation_data=ds_l_val.batch(self.batch_size),
                callbacks=callbacks,
            )
    ...
    def predict(self, data_gen_u, logits=False, y_labels=False):
        '''
        :param data_gen_u: tf.dataset generator, returning tuples (x) of image
            data
        '''
        ...
        if self.end_to_end:
            ...
            if self.multi_task:
                ...
                predicted_c, predicted_y = self.cb_model.predict(
                    data_gen_u.batch(self.batch_size)
                )
            ...
            if self.multi_task:
                predicted = self.concept_extractor.predict(
                    data_gen_u.batch(self.batch_size)
                )
                # Compute class values for every concept and concatenate the
                # results
                predicted = [
                    np.expand_dims(np.argmax(predicted[i], axis=-1), axis=-1)
                ...
                return (self._debinarize(predicted) > 0.5).astype(np.int32)
