...
class TopicModel(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.topic_vector = tf.Variable(
            initializer(shape=(n_channels, n_concepts)),
            trainable=True,
        )
        self.rec_vector_1 = tf.Variable(
            initializer(shape=(n_concepts, rec_param)),
            trainable=True,
        )
        self.rec_vector_2 = tf.Variable(
            initializer(shape=(rec_param, n_channels)),
            trainable=True,
        )
    ...
    def _compute_loss(self, x, y_true, is_training, with_losses=True):
        ...
        topic_prob_mask = K.cast(
            K.greater(topic_prob_n, self.threshold),
            tf.float32,
        )
        ...
        reg_loss_1 = tf.reduce_mean(
            tf.nn.top_k(
                K.transpose(K.reshape(topic_prob_n, (-1, self.n_concepts))),
                k=self.top_k,
                sorted=True
            ).values
        )
        ...
        reg_loss_2 = tf.reduce_mean(
            K.dot(
                K.transpose(topic_vector_n),
                topic_vector_n
            ) - np.eye(self.n_concepts)
        )

        total_loss = (
            log_prob_loss - self.lambda1*reg_loss_1 + self.lambda2*reg_loss_2
        )

        # Compute the regularization similarity metric to track
        sim_metric = tf.reduce_mean(
            tf.nn.top_k(
                K.transpose(K.reshape(topic_prob_n, (-1, self.n_concepts))),
                k=self.top_k,
                sorted=True
    ...
    def train_step(self, data):
        ...
        self.update_metrics(
            [("loss", loss), ("mean_sim", mean_sim), ("accuracy", acc)]
        )
