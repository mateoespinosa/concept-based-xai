...
class ConceptWhiteningLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, num_features):
        ...
        # Current implementation works with 1 group only (does not support
        # group whitening)
        self.num_groups = 1
        ...
        self.running_wm = tf.broadcast_to(
            tf.eye(self.num_channels),
            broadcast_shape,
        )
        # Running rotation matrix
        self.running_rot = tf.broadcast_to(
            tf.eye(self.num_channels),
            broadcast_shape,
        )
        # Sum of Gradient matrix
        self.sum_G = tf.zeros(
            (self.num_groups, self.num_channels, self.num_channels)
        )
    ...
    def _update_rotation_matrix(self):
        ...
        for i in range(2):
            ...
            # GR^T - RG^T
            A = tf.einsum('gin,gjn->gij', G, R) - tf.einsum(
                'gin,gjn->gij',
                R,
                G,
            )
    ...
    def call(self, X, training=False):
        ...
        # "mode" specifies the concept index, which specifies which gradient
        # matrix column to update
        if self.mode >= 0:
            if self.activation_mode == 'mean':

                self.sum_G = tf.Variable(self.sum_G)
                self.sum_G[:, self.mode, :].assign(
                    self.momentum * -tf.math.reduce_mean(X_hat, axis=[0, 3, 4])
                ...
                # Note: tensorflow does not currently support the 2D-unpool op
                # available in PyTorch. Hence, the implementation is a little
                # different here
                ...
                mean_maxpool = tf.reshape(
                    mean_maxpool,
                    [mean_maxpool.shape[0], size_R[0], size_R[2]],
                )
                ...
                raise NotImplementedError(
                    "Currently supporting only the pool_max option"
                )

        X_hat = tf.linalg.einsum('bgchw,gdc->bgdhw', X_hat, self.running_rot)
        return tf.reshape(X_hat, size_X)
    ...
    def _compute_whitened_activations(self, X, training):
        ...
        For the idea of "groups" of activations, see
        https://arxiv.org/pdf/1804.08450.pdf
        ...
        if training:
            ...
            Sigma = eps*I + 1./m * tf.linalg.matmul(
                xc,
                tf.transpose(xc, perm=[0, 2, 1])
            )

            # Calculate trace-normalized covariance matrix using eqn. (4) in the
            # paper
            Sigma_Tr_rec = tf.math.reciprocal(tf.linalg.trace(Sigma))
            # Keep shape (g, 1, 1)
            Sigma_Tr_rec = tf.expand_dims(
                tf.expand_dims(Sigma_Tr_rec, axis=-1),
                axis=-1,
            )
