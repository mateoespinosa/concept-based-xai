...
def visualize_nearest_neighbours(
    ...
    topic_vec_n = topic_vec / (
        np.linalg.norm(topic_vec, axis=0, keepdims=True) + 1e-9
    )
    topic_prob = np.matmul(f_data_n, topic_vec_n)

    for i in range(n_concepts):
        # Note: topic_prob is an array of shape
        ...
        for jc, j in enumerate(ind):
            ...
            # Retrieve the subpart of the image corresponding to the activated
            # patch
