...
def multi_task_cnn(input_shape, num_concpet_values, concept_names=[]):
    ...
    for i, c in enumerate(num_concpet_values):
        l = tf.keras.layers.Dense(
            ...
            activation="softmax",
            name=(f"l{i}" if concept_names == [] else concept_names[i]),
        )(h)
...
def sigmoid_cnn(input_shape, num_concept_values, concept_names=[]):
    ...
    :param num_concpet_values: list of containing number of concepts values for
        each concepts (e.g. [3,6,40,32,3] for dSprites)
    :return: CNN architecture model with shared convolutional layers and
        multiple heads for respective task to predict multiple concept values in
        different outputs color, shape, scale, rotation, x and y positions
    '''
...
def conv_encoder(input_shape, num_latent):
    """
    CNN encoder architecture used in the 'Challenging Common Assumptions in
    the Unsupervised Learning of Disentangled Representations' paper
...
def deconv_decoder(output_shape, num_latent):
    """
    CNN decoder architecture used in the 'Challenging Common Assumptions in
    the Unsupervised Learning of Disentangled Representations' paper (
    https://arxiv.org/abs/1811.12359)
